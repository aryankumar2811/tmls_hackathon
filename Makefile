.PHONY: install fixtures backend frontend ingest demo lint test

install:          ## install python + node deps
	uv sync
	cd frontend && npm install

fixtures:         ## (re)generate the 4 scenario fixtures
	uv run python -m backend.app.fixtures.generate
...
demo:             ## one-shot: fixtures + ingest, then print next steps
	$(MAKE) fixtures
	$(MAKE) ingest
